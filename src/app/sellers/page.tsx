      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        {sellers.length === 0 ? (
          <div className="text-center py-20 text-muted-foreground">
            ...
            <p className="text-sm mt-2">Be the first to <Link href="/auth/signup?role=seller" className="text-[#e2472b] underline">start selling!</Link></p>
          </div>
        ) : (
                ...
                <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full">
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="font-semibold text-foreground text-lg">{seller.storeName}</h3>
                        {seller.cuisineType && (
                          <p className="text-sm text-muted-foreground">{seller.cuisineType}</p>
                        )}
                      </div>
                      {seller.ratingAvg && (
                    ...
                    {seller.bio && (
                      <p className="text-sm text-muted-foreground mt-2 line-clamp-2">{seller.bio}</p>
                    )}
