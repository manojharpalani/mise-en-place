  ...
  return (
    <Sheet open={open} onOpenChange={(v: boolean) => { if (!v) onClose() }}>
      <SheetContent
        ...
        style={{ background: '#ffffff', height: '92dvh' }}
      >
        {/* Header */}
        <SheetHeader className="px-4 pt-4 pb-3 border-b" style={{ borderColor: '#efe3c7' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ShoppingCart className="w-5 h-5" style={{ color: '#e2472b' }} />
              <SheetTitle className="font-heading text-lg" style={{ color: '#12241b' }}>
                Grocery List
              </SheetTitle>
            </div>
            <div className="flex items-center gap-2">
              {aggregated.length > 0 && checkedCount > 0 && (
                <button
                  ...
                  style={{ color: '#4d5747', background: '#fbf3df' }}
                >
                  Clear ({checkedCount})
                </button>
              )}
              <button
                ...
                style={{ color: '#4d5747', background: '#fbf3df' }}
              >
                {copied ? <Check className="w-4 h-4 text-[#e2472b]" /> : <Copy className="w-4 h-4" />}
              </button>
              <button onClick={fetchData} className="p-1.5 rounded-lg" style={{ color: '#4d5747', background: '#fbf3df' }}>
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                    m.id === activeMenuId
                      ...
                      : 'border-[#efe3c7] text-[#4d5747] bg-white'
                  }`}
                  style={m.id === activeMenuId ? { background: '#12241b', borderColor: '#12241b' } : {}}
                >
                  {m.label}
                </button>
              ))}
          ...
          <div className="mt-3">
            <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
              ...
              <button
                ...
                style={filterMode === 'week' ? { background: '#e2472b', borderColor: '#e2472b' } : {}}
              >
                All Week
              </button>
              ...
              {days.map(({ day }) => (
                <button
                  ...
                  style={filterMode === 'day' && selectedDay === day ? { background: '#e2472b', borderColor: '#e2472b' } : {}}
                >
                  {day.slice(0, 3)}
                </button>
              ))}
              ...
              {days.length > 0 && data && data.dishes.length > 0 && (
                <span className="flex-shrink-0 w-px h-5 bg-[#efe3c7]" />
              )}
              ...
              {uniqueDishes.map((dish) => (
                <button
                  ...
                  style={filterMode === 'dish' && selectedDishId === dish.id ? { background: '#c43a20', borderColor: '#c43a20' } : {}}
                >
                  {dish.name}
                </button>
              ))}
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3">
              <Loader2 className="w-8 h-8 animate-spin" style={{ color: '#e2472b' }} />
              <p className="text-sm" style={{ color: '#4d5747' }}>Loading grocery list...</p>
            </div>
          ) : !data ? null : (
            <div className="px-4 py-4 space-y-2">
              ...
              {dishesWithoutIngredients.length > 0 && (
                <div className="rounded-xl p-3 mb-4 border" style={{ background: '#fff1d6', borderColor: '#efe3c7' }}>
                  <div className="flex items-start gap-3">
                    <Sparkles className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#c43a20' }} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium" style={{ color: '#12241b' }}>
                        {dishesWithoutIngredients.length} dish{dishesWithoutIngredients.length > 1 ? 'es' : ''} need ingredients
                      </p>
                      <p className="text-xs mt-0.5" style={{ color: '#4d5747' }}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {dishesWithoutIngredients.map(dish => (
                      <button
                        ...
                        style={{ borderColor: '#efe3c7', color: '#4d5747', background: 'white' }}
                      >
                        {generatingFor === dish.id ? (
                          <Loader2 className="w-3 h-3 animate-spin" />
                        ) : (
                          <Sparkles className="w-3 h-3" style={{ color: '#c43a20' }} />
                        )}
                        {dish.name}
                      </button>
                    ))}
              ...
              {aggregated.length === 0 && dishesWithoutIngredients.length === 0 && (
                <div className="text-center py-16">
                  <ShoppingCart className="w-12 h-12 mx-auto mb-3 opacity-20" style={{ color: '#e2472b' }} />
              ...
              {aggregated.length > 0 && (
                <div className="mb-4">
                  <div className="flex justify-between text-xs mb-1.5" style={{ color: '#4d5747' }}>
                    <span>{checkedCount} of {aggregated.length} checked</span>
                    <span>{Math.round(checkedCount / aggregated.length * 100)}%</span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: '#efe3c7' }}>
                    <div
                      className="h-full rounded-full transition-all"
                      style={{
                        width: `${Math.round(checkedCount / aggregated.length * 100)}%`,
                        background: 'linear-gradient(90deg, #e2472b, #c43a20)',
                      }}
                    />
                  </div>
                </div>
              ...
              {Array.from(grouped.entries()).map(([cat, items]) => {
                ...
                return (
                  <div key={cat} className="rounded-xl overflow-hidden border" style={{ borderColor: '#efe3c7' }}>
                    {/* Category header */}
                    <button
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-sm" style={{ color: '#12241b' }}>{label}</span>
                        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: '#efe3c7', color: '#4d5747' }}>
                          {catChecked}/{items.length}
                        </span>
                      </div>
                      {isCollapsed
                        ? <ChevronRight className="w-4 h-4" style={{ color: '#6f7768' }} />
                        : <ChevronDown className="w-4 h-4" style={{ color: '#6f7768' }} />
                      }
                    </button>
                        ...
                        {items.map((ing, idx) => {
                          ...
                          return (
                            <button
                              ...
                              className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors active:bg-[#fff1d6]"
                              style={{
                                borderTop: idx > 0 ? '1px solid #fbf3df' : undefined,
                              }}
                            >
                              {/* Checkbox */}
                              <div
                                ...
                                style={{
                                  borderColor: isChecked ? '#e2472b' : '#e5d8b8',
                                  background: isChecked ? '#e2472b' : 'white',
                                }}
                              >
                              ...
                              <span
                                ...
                                style={{ color: isChecked ? '#6f7768' : '#4d5747' }}
                              >
